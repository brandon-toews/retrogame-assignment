...
// Platform:	Independent
// Notes:		Uses 
//  format. The end of the filename indicates the number of frames e.g. "bat_4.png" or "tiles_10x10.png"
//********************************************************************************************************************************
...
// Platform:	Independent
// Notes:		The GameObject management is "opt in" as many will want to create their own Game
//  class
//********************************************************************************************************************************
...
struct GameObject
{
	GameObject( int type, Point2D pos, int collisionRadius, int spriteId, Point2f size, Point2f startMatrix, Point2f endMatrix );
	...
	Point2D startMatrix{ 0.0f, 0.0f };

	Point2D endMatrix{ 0.0f, 0.0f };
...
namespace Play
{
	...
	bool IsLeavingDisplayArea( GameObject& obj, Direction dirn = ALL, int objectType = 0 );
...
GameObject::GameObject( int type, Point2f newPos, int collisionRadius, int spriteId = 0, Point2f size = { 0.0f, 0.0f }, Point2f startMatrix = { 0.0f, 0.0f }, Point2f endMatrix = { 0.0f, 0.0f })
	: type( type ), pos( newPos ), radius( collisionRadius ), spriteId( spriteId ), size( size ), startMatrix( startMatrix ), endMatrix( endMatrix )
{
...
namespace Play
{
	...
	int CreateGameObject( int type, Point2f newPos, int collisionRadius, const char* spriteName )
	{
		...
		Point2f size = { Play::GetSpriteWidth(spriteId), Play::GetSpriteHeight(spriteId) };

		Point2f startMatrix = { newPos.x - (size.width / 2), newPos.y - (size.height / 2) };

		Point2f endMatrix = { startMatrix.x + size.width, startMatrix.y + size.height };

		// Deletion is handled in DestroyGameObject()
		GameObject* pObj = new GameObject( type, newPos, collisionRadius, spriteId, size, startMatrix, endMatrix );
		int id = pObj->GetId();
	...
	void UpdateGameObject( GameObject& obj, bool bWrap, int wrapBorderSize )
	{
		if( obj.type == -1 ) return; // Don't update noObject 
			...
			obj.size = { Play::GetSpriteWidth(obj.spriteId), Play::GetSpriteHeight(obj.spriteId) };
			obj.startMatrix = { obj.pos.x - (obj.size.width / 2), obj.pos.y - (obj.size.height / 2) };
			obj.endMatrix = { obj.startMatrix.x + obj.size.width, obj.startMatrix.y + obj.size.height };
	...
	bool IsLeavingDisplayArea( GameObject& obj, Direction dirn, int objectType )
	{
		if( obj.type == -1 ) return false; // Not for noObject
		...
		case 0:
			...
			if (dirn != VERTICAL)
			{
				if (obj.pos.x - spriteOrigin.x < -80 && obj.velocity.x < 0)
					return true;

				if (obj.pos.x + spriteSize.width - spriteOrigin.x > (pbuf.GetWidth() + 80) && obj.velocity.x > 0)
					return true;
			}

			if (dirn != HORIZONTAL)
			{
				if (obj.pos.y - spriteOrigin.y < -80 && obj.velocity.y < 0)
					return true;

				if (obj.pos.y + spriteSize.height - spriteOrigin.y > (pbuf.GetHeight() + 80) && obj.velocity.y > 0)
					return true;
			}
